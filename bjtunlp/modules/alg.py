...
def eisner2o(scores, mask):
    ...
    s_arc = s_arc.permute(2, 1, 0).cpu()
    # [seq_len, seq_len, seq_len, batch_size]
    s_sib = s_sib.permute(2, 1, 3, 0).cpu()
    s_i = torch.full_like(s_arc, float('-inf')).cpu()
    s_s = torch.full_like(s_arc, float('-inf')).cpu()
    s_c = torch.full_like(s_arc, float('-inf')).cpu()

    # s_i s_s  s_c  s_sib  s_arc lens
